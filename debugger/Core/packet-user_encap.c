...
static user_encap_t user0_encap;
...
static void dissect_user(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree) {
    ...
    if (!encap && pinfo->match_uint == WTAP_ENCAP_USER0) {
        /*
         * Special-case DLT_USER2 - Apple hijacked it for use as DLT_PKTAP.
         * The user hasn't assigned anything to it, so default it to
         * the PKTAP dissector.
...
void proto_reg_handoff_user_encap(void)
{
    ...
    user0_encap.encap = WTAP_ENCAP_USER0;
    user0_encap.payload_proto_name = g_strdup("netide");
    user0_encap.payload_proto = find_dissector("netide");
    user0_encap.header_proto_name = g_strdup("");
    user0_encap.header_proto = NULL;
    user0_encap.trailer_proto_name = g_strdup("");
    user0_encap.trailer_proto = NULL;
    user0_encap.header_size = 0;
    user0_encap.trailer_size = 0;
